...
class MergeSortTest {
    ...
    fun is_sort_int() {
        ...
        list.insert_next(0)
        list.insert_next(2)
        val ret = list.mg_sort(Comparator(compare_int))
        ret.print()
    ...
    fun is_sort_string() {
        ...
        list.insert_next("C")
        list.insert_next("B")
        val ret = list.mg_sort(Comparator(compare_string))
        ret.print()
        verify_string(ret, "ABCacdef")
    }

    @Test
    fun is_sort_int_array() {
        val list = arrayListOf(1, 4, 3, -1, 5, -2, 0, 2)
        list.mg_sort(Comparator(compare_int))
        Assert.assertEquals(list, arrayListOf(-2, -1, 0, 1, 2, 3, 4, 5))
    }
